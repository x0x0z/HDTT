...
# =========================
# HDTTStackingClassifier (Complete Definition - Reverted to LogisticRegression Meta-Learner)
# =========================
...
def plot_with_bands(df_runs, out_prefix):
    ...
    if "HDTT(RF+XGB+LogReg)" in palette:
        palette["HDTT(RF+XGB+LogReg)"] = (0.121, 0.466, 0.705)
    marker_map = {
...
def main():
    ...
    path_benign = "dataset1_simulated.csv"
    ...
    for i in range(N_RUNS):
        ...
        for nl in NOISE_LEVELS:
            print(f"Running iteration {i+1}/{N_RUNS}, Noise Level: {nl}")
            all_rows.extend(run_once(df, run_seed=run_seed, noise_level=nl))

    df_runs = pd.DataFrame(all_rows)

    os.makedirs("reports", exist_ok=True)
    runs_path = "reports/stacking_10runs_robust_runs.csv" # Updated filename
    ...
    agg_path = "reports/stacking_10runs_robust_agg.csv" # Updated filename
    agg.to_csv(agg_path, index=False)

    png, pdf = plot_with_bands(df_runs, out_prefix="figures/stacking_10runs_robust_final") # Updated filename
